...
export function ShortcutsModal() {
  ...
  return (
    ...
    >
      <div
        ref={modalRef}
        className="bg-background border border-border rounded-xl w-[420px] max-h-[80vh] overflow-hidden shadow-[0_16px_48px_rgba(0,0,0,0.5)]"
              ...
              <div className="space-y-1.5">
                {group.shortcuts.map((shortcut) => (
                  <div key={shortcut.description} className="flex items-center justify-between">
                    <span className="text-[12.5px] text-muted-foreground">{shortcut.description}</span>
                    <div className="flex gap-1">
                      {shortcut.keys.map((key, i) => (
                        <kbd
                          key={i}
                          className="min-w-[24px] h-6 px-1.5 rounded border border-border bg-muted text-[11px] font-mono text-muted-foreground flex items-center justify-center"
                        >
                          {key}
                        </kbd>
                      ))}
