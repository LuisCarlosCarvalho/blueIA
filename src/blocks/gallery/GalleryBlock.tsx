...
function ImageCard({ image, tall }: { image: GalleryImage; tall?: boolean }) {
  return (
    <div className={`rounded-lg overflow-hidden border border-border group ${tall ? 'row-span-2' : ''}`}>
      {image.src ? (
      ...
      {image.caption && (
        <div className="px-3 py-2 bg-secondary text-[11px] text-muted-foreground">{image.caption}</div>
      )}
    </div>
  )
}
